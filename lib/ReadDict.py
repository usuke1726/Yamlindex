...
def ReadDict(data: dict, book_alias: str, book_id: str, book_prefix: str):
    n = __DictNum(data)
    if n == 0:
        Log(f"索引語が1つもありません ({book_alias})")
        return
    with Progress(n, f"ReadDict: {book_alias} ({book_id})") as prog:
        __ReadDict_proc(prog, data, book_alias, book_id, book_prefix, [])

def __ReadDict_proc(prog, data: dict, book_alias: str, book_id: str, book_prefix: str, ref: list):
    for k, v in data.items():
        k = str(k)
        new_ref = ref + [k]
        if type(v) == dict:
            __ReadDict_proc(prog, v, book_alias, book_id, book_prefix, new_ref)
        elif type(v) == list:
            if len(v) == 0:
            ...
            for row in v:
                try:
                    if type(row) == dict:
                        __ReadDict_proc(prog, row, book_alias, book_id, book_prefix, new_ref)
                    else:
                        __AppendWord(row, book_alias, book_id, book_prefix, new_ref)
                        prog.step()
...
def __AppendWord(row, book_alias: str, book_id: str, book_prefix: str, ref: list):
    if type(row) == str:
        __AppendWord_str(row, book_alias, book_id, book_prefix, ref)
    elif type(row) == list:
        __AppendWord_list(row, book_alias, book_id, book_prefix, ref)
    else:
        raise WordDataError(f"型が文字列でもリストでもありません: {row}")

def __AppendWord_str(s: str, book_alias: str, book_id: str, book_prefix: str, ref: list):
    if len(s.strip()) == 0:
...
def __AppendWord_list(l: list, book_alias: str, book_id: str, book_prefix: str, ref: list):
    n = len(l)
    if n == 0:
        raise WordDataError(f"空のリストです ({book_alias}[{'/'.join(ref)}])")
    comp, disp = DefParts(l[0], book_prefix)
    if n >= 3:
        desc = l[2]
        if type(desc) == str:
            desc = [desc]
...
def DefParts(d, book_prefix: str):
    if type(d) == str:
        d = f"{book_prefix}{d}"
        ...
        disp = d[0].strip()
        disp_with_prefix = f"{book_prefix}{disp}"
        comp = (ToHiragana(disp_with_prefix) if len(d) == 1 else f"{book_prefix}{d[1]}").strip()
        if disp == '':
            raise WordDataError(f"空の索引語が渡されました: [{repr(disp)}, {repr(comp)}]")
        elif comp == '':
            raise WordDataError(f"空のふりがなが渡されました: [{repr(disp)}, {repr(comp)}]")
        return comp, disp_with_prefix
    else:
